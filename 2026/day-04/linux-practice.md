...
- ####sudo systemctl status docker: Checks whether the docker is active (running) oor not.
...
- ####sudo systemctl list-units-files | grep docker: Shows all installed Docker-related systemd units and their startup status (enabled, disabled, static, etc.).
                                                     grep docker filters the output to only show Docker services.
...
- ####journalctl -u <service>: Shows the logs for a specific systemd service.

- ####journalctl -u <service> --since "1 hour ago":Shows logs for a specific service from the last 1 hour.
