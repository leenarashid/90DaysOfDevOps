...
![Prometheus Target](images/2.1.png)

![Prometheus Target](images/2.2.png)
...
![Prometheus Alerts](images/4.1.png)
...
![Inactive Status Of Alerts](images/4.2.png)

![Firing Alert Status](images/4.3.png)
...
![Above 100](images/check1.png)

![Above 50 (Fire Alert)](images/check2.png) (just to check,I changed the threshold)
...
- Defined in Prometheus alert rule files (YAML).
- Use PromQL on Prometheus metrics only.
- Notifications are sent through Alertmanager. 
- Best for infrastructure and application metric monitoring.



`Grafana Alerts `

- Created in the Grafana UI.
- Can use multiple data sources (Prometheus, Loki, CloudWatch, Elasticsearch, etc.).
- Uses Grafana's Unified Alerting and notification policies.
- Best for dashboard-based and cross-data-source alerts.

`When to use each:`
- `Use Prometheus Alerts`: For metric-based alerts (CPU, memory, disk, service down).
- `Use Grafana Alerts`: For alerts combining multiple data sources or dashboard-based monitoring (e.g., Prometheus metrics + Loki logs).
---
...
![Docker Containers](images/last.png)
...
OpenTelemetry Collector receives telemetry data from applications through the OTLP protocol.
...
Applications send metrics, logs, and traces to these endpoints.

### Processors
Processors modify or prepare telemetry data before exporting.

- Batch processor groups telemetry data together.
- It improves performance by sending data in batches.
...
- Prometheus server scrapes these metrics.

Logs and Traces:
- Debug exporter prints telemetry data for testing and troubleshooting.

### Pipelines

Metrics Pipeline:
Application → OTLP Receiver → Batch Processor → Prometheus Exporter → Prometheus → Grafana

Traces Pipeline:
Application → OTLP Receiver → Batch Processor → Debug Exporter
...
Below is the complete architectural layout of the observability stack. It visually details the Metrics (Blue), Logs (Orange), Traces (Purple), and Client Request (Green) pipelines.





![Full Stack Architecture Diagram](images/Architecture-diagram.png)